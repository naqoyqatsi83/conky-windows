...
conky::simple_config_setting<bool> out_to_x = conky::deprecated(
    ...
    "set `output_backend = 'x11'` instead");
...
std::set<output_t> &output_backends() {
  if (resolved_outputs.empty()) {
    CRIT_ERR("output_backends are incorrect before backend initialization is attempted");
  }
...
bool output_is(output_t output) {
  if (!outputs_ever_set) {
    CRIT_ERR("attempt to call output_is before any outputs were ever set is invalid; we don't have enough information yet");
  }
...
bool output_enabled(output_t output) {
  if (resolved_outputs.empty()) {
    CRIT_ERR("attempt to call output_enabled before backend initialization is attempted is invalid; we don't have enough information yet");
  }
